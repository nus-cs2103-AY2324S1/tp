...
/**
 * Changes the remark of an existing musician in the address book.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD
            + ": Edits the remark of the musician identified "
            + "by the index number used in the last musician listing. "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the musician in the filtered musician list to edit the remark
     * @param remark of the musician to be updated to
     */
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of musician data. We achieve that by working with the `Person` model. Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the musician’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a musician.

### Add a new `Remark` class

Create a new `Remark` in `seedu.address.model.musician`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each musician.
...
``` java
public PersonCard(Person musician, int displayedIndex) {
    //...
    remark.setText(musician.getRemark().value);
}
```
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person musicianToEdit = lastShownList.get(index.getZeroBased());
        Person editedMusician = new Person(
                musicianToEdit.getName(), musicianToEdit.getPhone(), musicianToEdit.getEmail(),
                musicianToEdit.getAddress(), remark, musicianToEdit.getTags());

        model.setPerson(musicianToEdit, editedMusician);
        model.updateFilteredPersonList(PREDICATE_SHOW_ALL_PERSONS);

        return new CommandResult(generateSuccessMessage(editedMusician));
    }
     ...
     * {@code musicianToEdit}.
     */
    private String generateSuccessMessage(Person musicianToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, musicianToEdit);
    }
```
