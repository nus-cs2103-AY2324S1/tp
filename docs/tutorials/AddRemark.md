...
This command allows users of the AddressBook application to add optional remarks to people in their bookingPeriod book and edit it if required. The command should have the following format:
...
Let’s start by creating a new `RemarkCommand` class in the `src/main/java/seedu/bookingPeriod/logic/command` directory.
...
``` java
package seedu.bookingPeriod.logic.commands;

import seedu.bookingPeriod.model.Model;

/**
 * Changes the remark of an existing booking in the bookingPeriod book.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD
            + ": Edits the remark of the booking identified "
            + "by the index number used in the last booking listing. "
...
``` java
import static seedu.bookingPeriod.commons.util.CollectionUtil.requireAllNonNull;
//...
public class RemarkCommand extends Command {
    //...
    ...
    /**
     * @param index of the booking in the filtered booking list to edit the remark
     * @param remark of the booking to be updated to
     */
...
Create a `RemarkCommandParser` class in the `seedu.bookingPeriod.logic.parser` package. The class must extend the `Parser` interface.
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of booking data. We achieve that by working with the `Person` model. Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the booking’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a booking.

### Add a new `Remark` class

Create a new `Remark` in `seedu.bookingPeriod.model.booking`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each booking.

Simply add the following to [`seedu.bookingPeriod.ui.BookingCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-639834f1e05afe2276a86372adf0fe5f69314642c2d93cfa543d614ce5a76688).
...
:exclamation: You must delete AddressBook’s storage file located at `/data/addressbook.json` before running it! Not doing so will cause AddressBook to default to an empty bookingPeriod book!
...
``` java
public PersonCard(Person booking, int displayedIndex) {
    //...
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person bookingToEdit = lastShownList.get(index.getZeroBased());
        Person editedBooking = new Person(
                bookingToEdit.getName(), bookingToEdit.getPhone(), bookingToEdit.getEmail(),
                bookingToEdit.getAddress(), remark, bookingToEdit.getTags());

        model.setPerson(bookingToEdit, editedBooking);
        model.updateFilteredPersonList(PREDICATE_SHOW_ALL_PERSONS);

        return new CommandResult(generateSuccessMessage(editedBooking));
    }
     ...
     * {@code bookingToEdit}.
     */
    private String generateSuccessMessage(Person bookingToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, bookingToEdit);
    }
```
