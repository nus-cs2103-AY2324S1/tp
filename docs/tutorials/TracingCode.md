...
According to the sequence diagram you saw earlier (and repeated above for reference), the `UI` component yields control to the `Logic` component through a method named `execute`. Searching through the code base for an `execute()` method that belongs to the `Logic` component yields a promising candidate in `seedu.ccacommander.logic.Logic`.
...
A quick look at the `seedu.ccacommander.logic.Logic` (an extract given below) confirms that this indeed might be what we’re looking for.
...
1. _Step into_ the line where user input in parsed from a String to a Command, which should bring you to the `CcaCommanderParser#parseCommand()` method (partial code given below):
   ``` java
...
1. Similar to before, you can step over/into statements in the `LogicManager#execute()` method to examine how the control is transferred to the `Storage` component and what happens inside that component.
   ...
   <div markdown="span" class="alert alert-primary">:bulb: **Intellij Tip:** When trying to step into a statement such as `storage.saveCcaCommander(model.getCcaCommander())` which contains multiple method calls, Intellij will let you choose (by clicking) which one you want to step into.
   </div>
...
1. It appears that a `JsonAdaptedPerson` is created for each `Person` and then added to the `JsonSerializableCcaCommander`.
...
2.  What components will you have to modify to perform the following
    ...
    3.  Save the records in the CSV format instead

    4.  Add a new command

    5.  Add a new field to `Person`

    6.  Add a new entity to the records
