...
* `sort d/status` sorts the applicant list by status in the order `UNDECIDED`, `OFFERED`, `REJECTED`.
...
### `filter`: Filtering applicants by fields
...
UI mockup:
![Sort UI Mockup](images/user-guide/filter.png)
