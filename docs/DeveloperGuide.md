...
4. As a continuation to point 3, this leads to a new issue with commands that can modify the display list length/order such as `editc`, `findc`, `deletec`, `addc` and their meeting variants.
   ...
   - For the case of `editc` and `editm`, this is judged to not be an issue as the view commands still obey their definition of displaying the item at a specified list index, and it is more important for the detailed view to display updates dynamically.
   - For the case of `deletec`, `deletem`, `findc` and `findm`, the stored `Index` is set to null to prevent potential out-of-bounds error.
   - For the case of `addc` and `addm`, as this issue only occurs in cases where the newly created Person/Meeting is listed above the currently viewed instances, the stored `Index` is also set to null for consistency.
