...
//		ModelStubAcceptingPersonAdded modelStub = new ModelStubAcceptingPersonAdded();
		...
		assertEquals(String.format(GroupPersonCommand.MESSAGE_SUCCESS, validPerson.getName().toString(), Messages.format(validGroup)),
				commandResult.getFeedbackToUser());

		assertEquals(modelStub.groupsAdded.getGroup("CS2100"), validGroup);
		assertEquals(modelStub.personsAdded.stream().filter(g -> g.getName().toString().equals(validPerson.getName().toString())).findFirst().orElse(null), validPerson);
	}

	// no group
	@Test
	public void execute_personAndNoGroupAcceptedByModel_groupPersonUnSuccessful() throws Exception {
//		ModelStubAcceptingPersonAdded modelStub = new ModelStubAcceptingPersonAdded();
		ModelStubWithGroup modelStub = new ModelStubWithGroup();
		...
		assertThrows(CommandException.class, Messages.MESSAGE_NO_GROUP_WITH_NAME_FOUND,
				 () -> groupPersonCommand.execute(modelStub));
	}

//	 no person
	@Test
	public void execute_NoPersonAndGroupAcceptedByModel_groupPersonUnSuccessful() throws Exception {
//		ModelStubAcceptingPersonAdded modelStub = new ModelStubAcceptingPersonAdded();
		ModelStubWithGroup modelStub = new ModelStubWithGroup();
	...
	@Test
	public void execute_PersonInGroup_groupPersonUnSuccessful() throws Exception {
		ModelStubWithGroup modelStub = new ModelStubWithGroup();
		...
		assertThrows(CommandException.class, String.format("%s is already in this group: %s", "Alice Pauline", "CS2100"),
				() -> groupPersonCommand.execute(modelStub));
	}
	...
	private class ModelStub implements Model {
		...
		@Override
		public String addTimeToPerson(Name toAddPerson, ArrayList<TimeInterval> toAddFreeTime) throws AssertionError {
			throw new AssertionError("This method should not be called.");
		}
		...
		public String deleteTimeFromPerson(Name personName,
		                                   ArrayList<TimeInterval> listOfTimesToDelete) throws AssertionError {
			throw new AssertionError("This method should not be called.");
		}
		...
		public String deleteTimeFromGroup(Group group,
		                                  ArrayList<TimeInterval> toDeleteTime) throws AssertionError{
			throw new AssertionError("This method should not be called.");
		}
	...
	private class ModelStubWithGroup extends ModelStub {
		...
		public Pair<Person, Group> groupPerson(String personName, String groupName) throws CommandException {
			...
			Person person = personsAdded.stream().filter(g -> g.getName().toString().equals(personName)).findFirst().orElse(null);
